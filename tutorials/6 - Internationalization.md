...
Translations are handled by the `translationModule` module which is part of the `widget-core-library`. The module fetches the corresponding translations from JSON files inside the `i18n` folder. 
...
We can test translations by changing the `locale` parameter on `modkSetupData.json`

	...
	exports.default = {
	   ...
	   init: function init(defaultArgs) {
	      ...
	      return new Promise(function (resolve, reject) {
	         ...
	         return _this2.getFile('https://c3-static.kambi.com/sb-mobileclient/widget-api/' + _this2.expectedApiVersion + '/kambi-widget-api.js').then(function (content) {
	            ...
	            var applySetupData = function applySetupData(setupData) {
	               ...
	               _this2.addClasses(_this2.kambiDefaultClasses);
	
	               var translationPromise = _translationModule2.default.fetchTranslations(setupData.clientConfig.locale);
	
	               var operatorCssPromise = _this2.injectOperatorCss(_this2.apiVersions.wapi, _this2.config.customer, _this2.config.offering);
	
	               var customCssPromise = _this2.injectCustomCss(_this2.args.customCssUrl, _this2.args.customCssUrlFallback);
	
	               // most widgets don't need to wait for the CSS to be loaded
	               // so we keep a promise instead of waiting for it
	               _this2.cssLoadedPromise = Promise.all([operatorCssPromise, customCssPromise]);
	   ...
	   injectOperatorCss: function injectOperatorCss(wApiVersion, customer, offering) {
	      ...
	      return this.getFile(url).then(function (content) {
	         var tag = _this3.createStyleTag('operator-css', url);
	         var head = document.getElementsByTagName('head')[0];
	   ...
	   injectCustomCss: function injectCustomCss(customCssUrl, customCssUrlFallback) {
	      ...
	      var appendToHead = function appendToHead(url) {
	         var tag = _this4.createStyleTag('custom-css', url);
	         var head = document.getElementsByTagName('head')[0];
	         // custom CSS should be the LAST CSS in the page
	      ...
	      return this.getFile(customCssUrl).then(function (response) {
	         ...
	         if (customCssUrlFallback !== '') {
	            ...
	            return _this4.getFile(customCssUrlFallback).then(function (response) {
